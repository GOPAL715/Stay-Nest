...
An Airbnb-style property rental platform built as a full-stack monorepo. StayNest ships two React 19 frontends — a customer-facing web app and an admin/host portal — powered by a single Spring Boot 3.4.1 REST API backed by PostgreSQL.
...
│       └── vite.config.ts
├── services/
│   └── api/               # Spring Boot 3.4.1 REST API
│       ├── src/
│       ├── pom.xml
│       └── Dockerfile
├── docker-compose.yml
...
|---|---|---|
| Customer frontend | Vercel | `https://<customer-vercel-url>` |
| Admin frontend | Vercel | `https://<admin-vercel-url>` |
| Backend API | Render (Docker, free) | `https://<render-api-url>` |
| Database | Supabase PostgreSQL (free) | configured via `SPRING_DATASOURCE_URL` |
| Swagger | served by the API | `https://<render-api-url>/api/v1/docs` |
| Redis | not required | backend uses the in-memory cache unless Redis is explicitly enabled |

Render environment variables: `PORT` (provided by Render), `SPRING_DATASOURCE_URL`, `SPRING_DATASOURCE_USERNAME`, `SPRING_DATASOURCE_PASSWORD`, `JWT_SECRET`, `FRONTEND_CUSTOMER_URL`, `FRONTEND_ADMIN_URL`, `EMAIL_VERIFICATION_REQUIRED=false` (cloud demo), `PAYMENTS_DEMO_MODE=true`, `ADMIN_EMAIL`, `ADMIN_PASSWORD` (bootstrap only).

Vercel customer: `VITE_API_BASE_URL`, `VITE_EMAIL_VERIFICATION_REQUIRED=false`, `VITE_ADMIN_PORTAL_URL`. Vercel admin: `VITE_API_BASE_URL`.
